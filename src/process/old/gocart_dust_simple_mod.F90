...
module gocart_dust_simple_mod

  use catchem_constants, only : kind_chem, g=>con_g, pi=>con_pi
  use catchem_config, only: num_chem, p_dust_1, p_dust_2, num_emis_dust
  use dust_data_mod
...
CONTAINS
  ...
  subroutine gocart_dust_simple(ktau,dt,u_phy,              &
          v_phy,chem_arr,rho_phy,dz8w,smois,u10,         &
          v10,delp,erod,isltyp,area,       &
          emis_dust,dusthelp,num_soil_layers,start_month)
     ...
     REAL(kind=kind_chem), INTENT(INOUT ) ::  dusthelp
...
! -- volumetric soil moisture over porosity
    ...
    chem_arr(p_dust_1)=max(max_default,(tc(1)+.3125*tc(2))*converi)
    chem_arr(p_dust_2)=max(max_default,(.67*tc(2)+tc(3))*converi)
    dusthelp=max(max_default,tc(2)*converi)
...
end module gocart_dust_simple_mod
